...
8. `plan-rebuild/19-KNOWLEDGE-CHAT-V2-QUALITY-REMEDIATION-PLAN.md`
9. `docs/adr/0034-conversation-v2-contract-boundary.md`
10. `plan-rebuild/20-FIGMAMAKE-STANDALONE-ACCOUNTING-INTELLIGENCE-COMPLETION-PLAN.md`
11. `docs/adr/0035-standalone-file-evidence-and-artifact-workspace.md`
12. `plan-rebuild/08-V2-CONVERSATION-REBUILD-HANDOFF.md`
13. `plan-rebuild/04-WORKSPACE-REPO-COUNCIL-DECISION.md`
14. `plan-rebuild/07-CONVERSATION-INTELLIGENCE-BRAVOGEN-BENCHMARK.md`
...
- Ship `FigmaMake_UI` as the sole production frontend after parity/rollback evidence; do not delete
  `frontend-react` before the separately reviewed retention gate.
...
- Treat BRAVO as an external system of record. The current product accepts user-owned/versioned
  file evidence and has no BRAVO API/DB/browser connector, journal import output or ERP mutation.
  Any future connector or write-back requires a new owner ADR.
...
- Deliver Office output as a bounded Artifact Workspace after verified analysis: native editable
  artifacts, approved templates/brand locks, diff/preview/version/audit and benchmarked fidelity.
  Do not build a separate general Office Agent or select an Office engine before the relevant
  format benchmark passes.
...
- Preserve the completed WP19-00 frozen System A/B artifact before changing prompts, routing,
  retrieval or synthesis; do not infer that it proves a quality win.
- Complete the independent SME review of the owner-accepted WP19 fixture before implementing the
...
- The current product exposes no ERP mutation. Internal review/artifact transitions remain
  revision/idempotency and maker/reviewer gated; generated SQL/config is never executed.
