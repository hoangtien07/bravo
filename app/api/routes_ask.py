...
async def ask(
    ...
    # Chặn 'abstain rồi bịa tiếp' (fail-closed): MỞ ĐẦU bằng abstain -> cắt mọi phần đuôi
    # (model có lúc mở đầu abstain rồi vẫn tự sinh các bước từ kiến thức ngoài, không nguồn).
    # PREFIX-match, không substring — câu trả lời một-phần được phép nhắc 'không tìm thấy'
    # giữa chừng (council 2026-07-12 #3).
    is_abstain = answer.strip().lower().startswith(_ABSTAIN.lower()[:30])
    if is_abstain:
        answer = _ABSTAIN
