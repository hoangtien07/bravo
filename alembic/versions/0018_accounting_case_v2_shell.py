...
def upgrade() -> None:
    ...
    if not inspector.has_table("accounting_cases_v2"):
        op.create_table(
            "accounting_cases_v2",
            ...
            sa.Column(
                "scope", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")
            ),
    ...
    if not inspector.has_table("accounting_case_commands_v2"):
        op.create_table(
            ...
            sa.Column(
                "outcome", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")
            ),
            sa.Column(
                "created_at",
                sa.DateTime(timezone=True),
            ...
            sa.UniqueConstraint(
                "subject", "idempotency_key", name="uq_accounting_case_command_subject_key"
            ),
        )
        op.create_index(
            "ix_accounting_case_commands_v2_subject", "accounting_case_commands_v2", ["subject"]
        )
        op.create_index(
            "ix_accounting_case_commands_v2_outcome_case_id",
            "accounting_case_commands_v2",
            ["outcome_case_id"],
        )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_accounting_case_commands_v2_outcome_case_id", table_name="accounting_case_commands_v2"
    )
    op.drop_index(
        "ix_accounting_case_commands_v2_subject", table_name="accounting_case_commands_v2"
    )
